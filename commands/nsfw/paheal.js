...
module.exports = {
    ...
    async execute(client, message, args, Discord) {
        ...
        if (!message.channel.nsfw) return message.channel.send(hornyEmbed)
           ...
           Booru.search('rule34.paheal.net', tag_query, { limit: 1, random: true })
           .then(posts => {
             if (posts.length === 0) {
             ...
             for (let post of posts) {
                 embed_nsfw = new Discord.MessageEmbed()
                  ...
                  .addField(`Provided by Rule34.paheal.net`, `[Full-Res Image](${post.postView})`)
        ...
        if (message.content.includes('help')) {
            const helpEmbed = new Discord.MessageEmbed()
                .addField('Returns an image from rule34.paheal', '`nabe paheal <optional query>`')
